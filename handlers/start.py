...
def start(message: Message) -> None:
    if new_user(message.from_user.id):
        bot.send_message(message.chat.id, 'Доброго дня, вельмешановне панство!\n'
    ...
    bot.send_message(message.chat.id,
                     'Виберіть що хочете виконати',
                     reply_markup=keyboard)


@bot.callback_query_handler(func=lambda data: data.data in ('weather', 'change_city', 'help'))
